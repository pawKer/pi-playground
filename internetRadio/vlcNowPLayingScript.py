    ...
    if(nowPlaying != 'UNKNOWN'):
        ...
        if(songTitle != 'UNKNOWN' and
           songArtist != 'UNKNOWN'):
            ...
            songz = '%s - %s' % (songTitle, songArtist)
#            return songz
...
def writeSongInfoToFile( songInfo, separator ):
    ...
    if(currentSongInfo != songInfo):
        currentSongInfo = htmlParser.unescape(songInfo)
...
if __name__ == '__main__':
    ...
    while 1:
        if time.time() < timeout:
            oldSong = currentSongInfo
            getInfo()
            if oldSong != currentSongInfo:
              sphd.clear()
	      sphd.show()
            timeout = time.time() + 60*1
	#sphd.write_string(" >> " + currentSongInfo.lower() + " << ", x=0, y=0, font=font5x7, brightness=0.1)
        sphd.write_string(" >> " + currentSongInfo.lower() + " << ")
